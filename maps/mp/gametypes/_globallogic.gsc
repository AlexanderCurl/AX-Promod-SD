...
spawnPlayer()
{
   ...
   //stats
   self thread showStats();
   if(self.sessionstate == "playing"){
      self setStat(3132,self getStat(3132)+1);
   }
}
   ...
   if ((level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) && !level.forcedEnd)
	 {
        ...
        if ( isDefined( game["PROMOD_KNIFEROUND"] ) && game["PROMOD_KNIFEROUND"] )        
          level.bb_lastkill[4] = false;

        //Round Winning Kill here!//
        if(isDefined(level.bb_lastkill)&&level.bb_lastkill[4] == true){
            //thread restoreGameSpeed();
            for(i=0; i<level.players.size; i++) {
              player=level.players[i];
              player thread maps\mp\gametypes\_killcam::roundwinningkill(level.bb_lastkill[0], level.bb_lastkill[1], level.bb_lastkill[2], [], level.bb_lastkill[3]);
		...
		if (roundSwitching && level.teamBased)
          ...
          //BB Half Time Sniper Free Maker
          thread maps\mp\gametypes\sd::makeSnipersFree();
...
checkScoreLimit ()
{
   if ((!isDefined (game["state"]) || game["state"] != "playing") || level.scoreLimit <= 0
	   || (level.teamBased && game["teamScores"]["allies"] < level.scoreLimit && game["teamScores"]["axis"] < level.scoreLimit) || (!level.teamBased
																																	&& (!isPlayer (self) || self.score < level.scoreLimit)))
	  return;
   [[level.onScoreLimit]] ();
}
		...
		if (playerCounts["allies"] == playerCounts["axis"])
		  {
            ...
            //BB Team Balance
				if ( isPlayer( self ) 
                 ...
                 && self.pers["team"] != "none" 
                 && ( self.sessionstate == "playing" || self.sessionstate == "spectator" ) )
...
updateTeamStatus ()
{
   ...
   for (i = 0; i < level.players.size; i++)
	  if (level.players[i].pers["team"] == "allies" || level.players[i].pers["team"] == "axis")
		 level.players[i] setClientDvars ("self_alive", level.aliveCount[level.players[i].pers["team"]], "opposing_alive",
										  level.aliveCount[maps\mp\gametypes\_gameobjects::getEnemyTeam (level.players[i].pers["team"])]);
   if (isDefined (level.scorebot) && level.scorebot)
	 {
		level.allies_team = "";
		level.axis_team = "";
...
playTickingSound() {
    ...
    level endon("game_ended");
   for(;;) {
      if (level.bb_bombTimer <= (level.bombTimer - 10)) {
            self playSound("ui_mp_suitcasebomb_timer");
            wait 1;
      }
      else {
            self playSound("ui_mp_suitcasebomb_timer");
            wait ((level.bombTimer - level.bb_bombTimer)/10);
      }
   }
...
startGame ()
{
   ...
   if ((isDefined (game["PROMOD_MATCH_MODE"]) && game["PROMOD_MATCH_MODE"] == "match" || getDvarInt ("promod_allow_strattime") && isDefined (game["CUSTOM_MODE"]) && game["CUSTOM_MODE"])
	   && level.gametype == "sd")
   ...
   if (!self maySpawn ())
	 {
		...
		if ((level.roundLimit > 1 && game["roundsplayed"] >= (level.roundLimit - 1))
			|| (level.scoreLimit > 1 && level.teambased && game["teamScores"]["allies"] >= level.scoreLimit - 1 && game["teamScores"]["axis"] >= level.scoreLimit - 1))
...
Callback_StartGameType ()
{
   ...
   if ((isDefined (game["PROMOD_MATCH_MODE"]) && game["PROMOD_MATCH_MODE"] == "match" || getDvarInt ("promod_allow_readyup") && isDefined (game["CUSTOM_MODE"]) && game["CUSTOM_MODE"])
	   && (!game["roundsplayed"] && !game["promod_first_readyup_done"] || (game["SCORES_ATTACK"] > 0 || game["SCORES_DEFENCE"] > 0)))
...
checkRoundSwitch ()
{
   ...
   if (game["roundsplayed"] % level.roundswitch == 0)
	 {
		if ((isDefined (game["PROMOD_MATCH_MODE"]) && game["PROMOD_MATCH_MODE"] == "match" || getDvarInt ("promod_allow_readyup") && isDefined (game["CUSTOM_MODE"]) && game["CUSTOM_MODE"])
			&& game["promod_first_readyup_done"])
		   game["promod_do_readyup"] = true;
...
Callback_PlayerConnect ()
{
   ...
   self setClientDvars ("cg_hudGrenadeIconMaxRangeFrag", int (!level.hardcoreMode) * 250, "cg_drawcrosshair", int (!level.hardcoreMode), "cg_drawSpectatorMessages", 1, "ui_hud_hardcore",
						level.hardcoreMode, "fx_drawClouds", 0, "ui_showmenuonly", "", "self_ready", "");
   ...
   if (!isDefined (self.pers["team"]))
	 {
      ...
      self thread settings\serverinfo::welcome_first();   
      self thread settings\serverinfo::news_actual(); 

      //BAD BOYS - Szerver welcome ////////////////////
      self setclientdvar ("g_scriptMainMenu", game["menu_team"]);

      //szerverinfók frissítése
      settings\serverinfo::welcome_first();
      settings\serverinfo::news_actual();

      //megfelelő menü megnyitása
      roundspl = self getStat(3132);

      if(roundspl <= 1 && getDvarInt("boys_welcome_menu") == 1){
         //Első belépőés
         self openMenu (game["menu_boys_welcome"]);
      }else{
         //nem első belépés
         lastNewsId = getDvarInt("boys_news_number");
         newsid = self getStat(3055);
         if( newsid < lastNewsId && getDvarInt("boys_news_menu") == 1){
            //hírek ha van új
            self setStat(3055,lastNewsId);
		      self openMenu (game["menu_boys_news"]);
         }else{
            //normál indulás
            self openMenu (game["menu_team"]);
         }
...
{
   //BB RWKILLCAM TILTÁS DISCONNECT ESTETÉN
   if(isDefined(level.bb_lastkill))
      level.bb_lastkill[4] = false;
...
Callback_PlayerDamage (eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime)
{
   if (!isDefined (level.rdyup))
	  level.rdyup = false;
   if (getDvarInt ("g_knockback") != 1000 || isDefined (game["state"]) && game["state"] == "postgame" || self.sessionteam == "spectator" || isDefined (game["PROMOD_MATCH_MODE"])
	   && game["PROMOD_MATCH_MODE"] == "strat" && isDefined (self.flying) && self.flying || isDefined (level.bombDefused) && level.bombDefused || isDefined (level.bombExploded) && level.bombExploded
	   && self.pers["team"] == game["attackers"] || isDefined (game["PROMOD_KNIFEROUND"]) && game["PROMOD_KNIFEROUND"] && sMeansOfDeath != "MOD_MELEE" && sMeansOfDeath != "MOD_FALLING"
	   && !level.rdyup)
	  return;
   if (isDefined (eAttacker) && isPlayer (eAttacker) && isPlayer (self) && eAttacker.sessionstate == "playing" && isDefined (iDamage) && isDefined (sMeansOfDeath) && sMeansOfDeath != ""
	   && (sMeansOfDeath == "MOD_RIFLE_BULLET" || sMeansOfDeath == "MOD_PISTOL_BULLET"))
   ...
   if (!(iDFlags & level.iDFLAGS_NO_PROTECTION))
	 {
		if ((isSubStr (sMeansOfDeath, "MOD_GRENADE") || isSubStr (sMeansOfDeath, "MOD_EXPLOSIVE") || isSubStr (sMeansOfDeath, "MOD_PROJECTILE")) && isDefined (eInflictor)
			&& game["PROMOD_MATCH_MODE"] != "match" && eInflictor.classname == "grenade" && ((self.lastSpawnTime + 3500) > getTime () && distance (eInflictor.origin, self.lastSpawnPoint.origin) < 250
																							 || !isDefined (eAttacker.pers["class"])))
		   return;
		...
		if (isDefined (eAttacker) && eAttacker != self)
		  {
           ...
           ///BAD BOYS
               eAttacker thread showDamage(iDamage,iDFlags,self);
           ///
        }
   ...
   if (self.sessionstate != "dead")
	 {
		...
		logPrint ("D;" + self getGuid () + ";" + self getEntityNumber () + ";" + self.pers["team"] + ";" + self.name + ";" + lpattackGuid + ";" + lpattacknum + ";" + lpattackerteam + ";" +
...
Callback_PlayerKilled (eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
   ...
   logPrint ("K;" + self getGuid () + ";" + self getEntityNumber () + ";" + self.pers["team"] + ";" + self.name + ";" + lpattackguid + ";" + lpattacknum + ";" + lpattackerteam + ";" + lpattackname +
   ...
   if (sWeapon == "none")
	  doKillcam = false;
   killcamentity = -1;
   if( isDefined(eInflictor) && level.bb_grenadecam )
    {
      if(isDefined(eInflictor.killCamEnt)&&sWeapon=="frag_grenade_mp")
      {
    ...
    if(!isDefined(level.bb_lastkill))
        level.bb_lastkill = [];
    level.bb_lastkill[0] = lpattacknum;
    level.bb_lastkill[1] = killcamentity;
    level.bb_lastkill[2] = sWeapon;
    level.bb_lastkill[3] = attacker;
    if(attacker == self || !doKillcam){
       level.bb_lastkill[4] = false;
    }else{
       level.bb_lastkill[4] = true;
    }

    //Statisztika gyűjtés
      attacker setStat(3130,attacker getStat(3130)+1); //kills
      self setStat(3131,self getStat(3131)+1); //deats
...
//BAD BOYS - GYORSULÓ CSIPOGÁS
bb_bombcountback(){
   self endon("death");
    self endon("stop_ticking");
    level endon("game_ended");
   //iprintlnbold(level.bombTimer);
   for(;;) {
      if (level.bb_bombTimer <= (level.bombTimer -2)) {
         level.bb_bombTimer = level.bb_bombTimer + 1;
      }
      
      if (level.bb_bombTimer == (level.bombTimer -5)){
         //iprintlnbold(level.bombTimer);
          playSoundOnPlayers("mp_killstreak_heli");
      }
      wait 1;
   }
}
...
//Damage jelző
showDamage(dam,flags,attacker)
{
   if( flags & level.iDFLAGS_PENETRATION || !level.bb_dmgindicator )
      return;
...
restoreGameSpeed(){
    ...
    wait 0.1;
    setDvar("timescale", 1);
    normalTimeScale=getDvarInt("boys_def_timescale");
    if(normalTimeScale>0.3)
      setDvar("timescale", normalTimeScale);*/
}

//Statisztikák mutatása késkörben
showStats(){
    wait 5;
   player = self;
   if (isDefined (game["PROMOD_KNIFEROUND"])
             && game["PROMOD_KNIFEROUND"]){
      kills=player getStat(3130);
      deaths=player getStat(3131);
      rounds=player getStat(3132);
      player iprintln("^3[Statistics]");
      wait .5;
      player iprintln("Overall kills: "+kills);
      wait .5;
      player iprintln("Overall deaths: "+deaths);
      wait .5;
      player iprintln("Rounds played: "+rounds);
   }
}
